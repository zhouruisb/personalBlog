...
<template>
  <div class="game-wrap" ref="gemeWrap" @mouseup="removeMove">
    ...
    <div
      ...
      v-show="nianshouHP"
    >
      <p style="color: 'red'">坤血: {{ nianshouHP }}</p>
      <img src="../assets/ji.png" class="nianshou-img" />
    </div>
    <!-- 鞭炮 -->
    <div
      class="paozhu"
    ...
    <!-- 面板区 -->
    <div class="panel-wrap" v-if="this.state == 0 ? true : false">
      <!-- 游戏面板 -->
      <div class="game-panel panel-item">
        <p class="game-title">击败坤兽</p>
    ...
    <div class="result-box" v-show="!nianshouHP">
      <div class="result-title">你好，2023！</div>
...
export default {
  ...
  data() {
    return {
      state: 0, //面板显示
      imgUrl: require("../assets/lanqiu.png"), //篮球地址
  ...
  methods: {
    ...
    touchM(e) {
      ...
      e = e || window.event;
      e.preventDefault();
      let movementX = e.touches[0].clientX - this.clientX;
    ...
    createBullet() {
      ...
      if (now - this.lastBulletTime > 1000 / this.frequency) {
        ...
        bullet.style.backgroundImage = `url(${this.imgUrl})`;
        ...
        let bulletMove = () => {
          ...
          if (
            bullet.offsetTop <= 100 &&
            bullet.offsetLeft >= this.nianshouLeft &&
            bullet.offsetLeft <= this.nianshouLeft + 100
          ) {
            // 年兽掉血
            this.nianshouHP -= this.damage;
            this.$store.commit("playAudio", require("../assets/mp3/a.mp3"));
